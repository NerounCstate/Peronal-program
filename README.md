2025.9.16这仅仅是一个C语言练习项目，期待以后的样子

2025.10.18
<font size=20>C语言学习正式告一段落</font>
庆祝庆祝，此项目谨以记录我这一个月的学习历程
我会在下一个README里继续完善自己，下次再见


%s用于读取字符串，会从起始读到'\0'结束，使用的是内存地址而非值
printf("%s",p);
相对的%c读取单个字符，%d读取ASCII码表值,传递值而非地址
printf("%c",'a');
printf("%d",3);/printf("%d",'A');
%p用来输出地址，除了指针不用*，其余值的地址要用&
printf("%p",p)/printf("%p",&a);

int str[] = {"......"}，无法被修改，只能被读取
int str[] = {'....','....',......}，可以被修改

读取一个字符串建议用指针

如果创建了一个数组，那么数组的名字本身就是一个指针。在请求输入的时候不需要&


2025.10.14
工作室要求1周之内学完C，我才学到指针数组，还有结构体、字符串之类的。。。。
在C语言课上和课余时间看视频吧
新建了一个文件bahe.c用来做工作室的小组报告，但是逻辑有问题只能由简单功能
VSCode可以对文件单独编译，比vs和dev之类得强多了哈哈哈哈


2025.10.10
修改了一个文件名，因为在vscode里似乎中文的c文件没法被编译，编译出来也是乱码和无法运行。


2025.10.8
我才想起来README更适合来做练习记录
好吧，这次不仅在fileName里写了一些新代码，后续还新建了一个练习文件，都是指针相关的练习。
明天就要坐高铁回学校了，但是新买的耳机买晚了，早点买就能赶上高铁了。只好邮到学校去以后用。是99块买的天使吉米BUNNY，传说是最有性价比的HiFi耳机。
appleID切换到港区后发现一个月竟然要68块钱，舍不得买。高中同学告诉我了他在用的音乐软件spotify。我去搜了一下，网上真是好评如潮，立马下了一个试试。歌曲全是全，就是音频的码率只有模糊的“低、中、高”和需要会员的“超高”。我以为高就是320kbps，超高就是差不多1536呗。我戴airpodspro2听了几首木柜子的歌，感觉很不对劲。正巧我好久没戴耳机好久没听歌、其他音乐也没有会员、pro3出来了以为会偷偷削弱pro2的音质，就买了spotify的年度会员。买完偶然看了个视频才知道tm高对应的是192kbps，超高只有tm320kbps。我还以为我是金耳朵呢，结果你给我搞这个？