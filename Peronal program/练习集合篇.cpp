...
//	int len = strlen(arr1);
//	printf("%s\n", arr0);
//	printf("%s\n", arr1);//只有在找到\0后才会结束运行“烫烫烫烫烫烫。。。”
//	printf("%d\n", len);//长度不会包括\0，但会找\0以结束计算长度
//		return 0;
//}

//int main() {
//	char arr0[5] = "1234";//只申请5个字符的长度，数组也只会读取并显示前4位，直到找到\0结束
//	printf("%s\n", arr0);
//	int len = strlen(arr0);
//	printf("%d\n", len);//字符类型可以直接打印输出，但其他的需要例如“%d\n”
//	return 0;
//}
...
//	scanf_s("%6s", input, (unsigned)sizeof(input)); // 最多读取6个字符，留一个位置给字符串结束符'\0'
